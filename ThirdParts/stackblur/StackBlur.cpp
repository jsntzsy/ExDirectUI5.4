...
)
{
	...
	if (step == 1)
	{
		...
		for (y = minY; y < maxY; y++)
		{
			...
			src_ptr = data + stride * y; // start of line (0,y)
			...
			xp = radius;
			if (xp > wm) xp = wm;
			src_ptr = data + y * stride + xp * 4; //   img.pix_ptr(xp, y);
			dst_ptr = data + y * stride; // img.pix_ptr(0, y);
			for (x = 0; x < w; x++)
			{
	...
	if (step == 2)
	{
		...
		for (x = minX; x < maxX; x++)
		{
			...
			yp = radius;
			if (yp > hm) yp = hm;
			src_ptr = data + 4 * x + yp * stride; // img.pix_ptr(x, yp);
			dst_ptr = data + 4 * x; 			  // img.pix_ptr(x, 0);
			for (y = 0; y < h; y++)
			{
				dst_ptr[0] = (sum_r * mul_sum) >> shr_sum;
				dst_ptr[1] = (sum_g * mul_sum) >> shr_sum;
...
)
{
	if (radius > 0 && radius < 255)
	{
		...
		stackblurJob(src, w, h, stride, radius, 1, 0, 1, stack);
		stackblurJob(src, w, h, stride, radius, 1, 0, 2, stack);
		delete[] stack;
	}
}

_stackblur_mt_info* stackblur_mt_begin(
	unsigned char* src,
	unsigned int w, unsigned int h, unsigned int stride,
	unsigned int radius, int cores)
{
	_stackblur_mt_info* info = nullptr;

	if (radius > 0 && radius < 255)
	{
		if (cores <= 1 || w <= cores || h <= cores)
		{
			stackblur(src, w, h, stride, radius);
		}
		else
		{
			info = new _stackblur_mt_info;
			...
			info->div = (radius * 2) + 1;
			info->stack = new unsigned char[info->div * 4 * cores];
...
void stackblur_mt(_stackblur_mt_info * info, int core, int step)
{
	stackblurJob(info->src, info->w, info->h, info->stride, info->radius, info->cores, core, step, info->stack + info->div * 4 * core);
}

void stackblur_mt_end(_stackblur_mt_info * info)
{
	delete[] info->stack;
	delete info;
}