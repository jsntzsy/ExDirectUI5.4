...
namespace ExDirectUI
{
	...
	// 图片加载函数
	ExAPngImageInfo* ExAPngOnLoad(void* data, bool is_file) MAYTHROW
	{
		...
		if (!png_get_valid(png_ptr_read, info_ptr_read, PNG_INFO_acTL)) {
			...
			for (UINT iFrame = 0; iFrame < apng->frame_count; iFrame++) {
				...
				if (png_get_valid(png_ptr_read, info_ptr_read, PNG_INFO_fcTL)) {
					...
					if (delay_den == 0 || delay_den == 100) { apng->delays[iFrame] = delay_num; }
					else {
						if (delay_den == 10) { apng->delays[iFrame] = delay_num * 10; }
						else {
							if (delay_den == 1000) { apng->delays[iFrame] = delay_num / 10; }
							else { apng->delays[iFrame] = delay_num * 100 / delay_den; }
						}
					}
				}
				else { apng->delays[iFrame] = 10; }
...

				{
					...
					default:  throw_ex(E_NOTIMPL, L"不支持的操作");  break;
					}

					png_bytep targetFrame = data + bytes_per_frame * iFrame;
					memcpy(targetFrame, curFrame, bytes_per_frame);

					lineDst = curFrame + info_ptr_read->next_frame_y_offset * bytes_per_row +
						4 * info_ptr_read->next_frame_x_offset;
